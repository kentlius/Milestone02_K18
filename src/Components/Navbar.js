...
function Navbar() {
  ...
  const sidebarStatus = sidebar
    ? `${styles.boxNav} ${styles.active}`
    : `${styles.boxNav}`;
  const textSidebarStatus = textSidebar
    ? `${styles.navbarText} ${styles.fadeIn}`
    : `${styles.fadeOut} ${styles.hidden}`;
  ...
  return (
    <IconContext.Provider value={{ color: "#fff" }}>
      <div className={styles.navBtn}>
        <Link
          to="#"
          className={`${barOutAnimation} ${initialBar} ${barInAnimation}`}
        >
          <FaIcons.FaBars
            onClick={() => {
              setFirstLoad(true);
